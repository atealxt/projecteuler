...
public class Problem54 extends Problem {
	...
	boolean p1Win(List<Card> cards1, List<Card> cards2) {
		...
		if (p1.rank != p2.rank) {
			return (p1.rank - p2.rank) > 0;
		}
		for (int i = p1.rankCards.size() - 1; i >= 0; i--) {
			if (p1.rankCards.get(i).value != p2.rankCards.get(i).value) {
				return (p1.rankCards.get(i).value - p2.rankCards.get(i).value) > 0;
			}
		}
		for (int i = p1.otherCards.size() - 1; i >= 0; i--) {
			if (p1.otherCards.get(i).value != p2.otherCards.get(i).value) {
				return (p1.otherCards.get(i).value - p2.otherCards.get(i).value) > 0;
			}
		}
		// deuce
		return false;
	...
	Poker recognize(List<Card> cards) {
		...
		for (int i = 0; i < cards.size(); i++) {
			...
			if (consecutive) {
				if (flush) {
					...
					return new Poker(Poker.RANK_STRAIGHT, cards);
				}
			}
		}
		...
		if (kinds.size() == 1) {
			...
			} else if (kinds.get(0).size() == 3) {
				return new Poker(Poker.RANK_THREE_OF_A_KIND, kinds.get(0), others);
			} else {
				return new Poker(Poker.RANK_ONE_PAIR, kinds.get(0), others);
			}
		}

		// kinds.size is 2
		if (kinds.get(0).size() == 3) {
			kinds.get(1).addAll(kinds.get(0));
			return new Poker(Poker.RANK_FULL_HOUSE, kinds.get(1), others);
		} else if (kinds.get(1).size() == 3) {
			kinds.get(0).addAll(kinds.get(1));
			return new Poker(Poker.RANK_FULL_HOUSE, kinds.get(0), others);
		} else {
			kinds.get(0).addAll(kinds.get(1));
			return new Poker(Poker.RANK_TWO_PAIRS, kinds.get(0), others);
		}
	}

	private static final String[] POKERS = { "8C TS KC 9H 4S 7D 2S 5D 3S AC",//
...
class Card implements Comparable<Card> {
	...
	private final static Map<String, Card> ALL = new HashMap<String, Card>();
	private final static String[] SUITS = new String[] { "H", "C", "S", "D" };
	private final static String[] TEXTS = new String[] { "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A" };
	String text;
	String suit;
	int value;

	public Card(String text, String suit, int value) {
		super();
		this.text = text;
		this.suit = suit;
		this.value = value;
	...
	static {
		...
		for (String text : TEXTS) {
			for (String suit : SUITS) {
				ALL.put(text + suit, new Card(text, suit, i + 2));
			}
			i++;
		}
	}
...
class Poker {
	final int rank;
	static final int RANK_ROYAL_FLUSH = 10;
	static final int RANK_STRAIGHT_FLUSH = 9;
	static final int RANK_FOUR_OF_A_KIND = 8;
	static final int RANK_FULL_HOUSE = 7;
	static final int RANK_FLUSH = 6;
	static final int RANK_STRAIGHT = 5;
	static final int RANK_THREE_OF_A_KIND = 4;
	static final int RANK_TWO_PAIRS = 3;
	static final int RANK_ONE_PAIR = 2;
	static final int RANK_HIGH_CARD = 1;
	final List<Card> rankCards = new ArrayList<Card>(5);
	final List<Card> otherCards = new ArrayList<Card>(5);

	public Poker(int rank, List<Card> rankCards) {
		this(rank, rankCards, new ArrayList<Card>());
	}

	public Poker(int rank, List<Card> rankCards, List<Card> otherCards) {
		super();
		this.rank = rank;
		this.rankCards.addAll(rankCards);
		this.otherCards.addAll(otherCards);
	}
}
